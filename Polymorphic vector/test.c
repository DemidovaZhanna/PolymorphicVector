//To run test.c use next command in console
//gcc test.c complex_unit.c vector.c utils.c -o test
...
int main() {
    ...
    if (IsTheSameVector(FilterI(&IsEven, v1), v2)) printf("Test 1 : SUCCESS\n");
    else printf("Test 1 : FAIL\n");
    if (IsTheSameVector(MapI(&IncI, v1), v7)) printf("Test 2 : SUCCESS\n");
    else printf("Test 2 : FAIL\n");
    Concatenation(v1, v2);
    if (IsTheSameVector(v1, v3)) printf("Test 3 : SUCCESS\n");
    else printf("Test 3 : FAIL\n");

    if (IsTheSameVector(FilterC(&IsPositiveRealAndImag, v4), FilterC(&IsPositiveRealAndImag, v5))) printf("Test 4 : SUCCESS\n");
    else printf("Test 4 : FAIL\n");
    if (IsTheSameVector(MapC(&IncReal, v4), v8)) printf("Test 5 : SUCCESS\n");
