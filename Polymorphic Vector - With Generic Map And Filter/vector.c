...
//Gives int* data
//Did that so that I don't need to write
//smth like that ((int*)GetData(v))[i]
...
Vector* Map(void* (*f)(void*), Vector* v){
    Eltype T = GetElType(v);
    if (T == INT) return MapI(f, v);
    else if (T == COMPLEX) return MapC(f, v);
    else UnknownType();

    //Function will never reach that
    //because UnknownType terminate the program with error
    return MakeVector(0, INT);
}
...
Vector* Filter(bool (*filter)(void*), Vector* v){
    Eltype T = GetElType(v);
    if (T == INT) return FilterI(filter, v);
    else if (T == COMPLEX) return FilterC(filter, v);
    else UnknownType();
...
int* IncI(int* x) { *x = *x + 1; return x; }
int* DecI(int* x) { *x = *x - 1; return x;}
int* AbsoluteI(int* x) { *x = abs(*x); return x; }
...
bool IsEven(int* x) { return (abs(*x) % 2 == 0 ? 1 : 0); }
bool IsOdd(int* x) { return (abs(*x) % 2 == 1 ? 1 : 0); }